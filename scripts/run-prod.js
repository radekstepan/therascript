...
async function cleanupLmsDaemon() {
  ...
  console.log('[RunProd Cleanup] Shutting down LM Studio daemon...');
  try {
    await execPromise(`"${lmsBinary}" daemon down`);
    console.log('[RunProd Cleanup] LM Studio daemon stopped.');
  } catch (err) {
    console.warn(
      `[RunProd Cleanup] Failed to stop LM Studio daemon: ${err.message}`
    );
  }
}
  ...
  if (appProcess && !appProcess.killed) {
    try {
      if (process.platform === 'win32') {
        ...
        console.log(
          `[RunProd] SIGTERM sent to concurrently process group (PGID: ${appProcess.pid}). Waiting up to 5s for graceful exit...`
        );
        const exited = await waitForProcessExit(appProcess, 5000);
        if (!exited) {
          console.warn(
            '[RunProd] Graceful shutdown timed out, escalating to SIGKILL.'
          );
          try {
          ...
          console.log(
            '[RunProd] Concurrently process group exited gracefully.'
          );
        }
      }
    } catch (killError) {
...
function createShutdownService(shutdownHandlerCallback) {
  ...
  server.listen(SHUTDOWN_PORT, SHUTDOWN_HOST, () => {
    console.log(
      `[RunProd ShutdownService] Listening on http://${SHUTDOWN_HOST}:${SHUTDOWN_PORT}/shutdown (allowing origin: ${UI_ORIGIN_FROM_ENV})`
    );
  });
