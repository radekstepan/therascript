"use client"
  ...
  import { CheckIcon, ChevronDownIcon, ChevronUpIcon } from "@radix-ui/react-icons" // Use Radix icons
  ...
  >(({ className, children, ...props }, ref) => (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex h-10 w-full items-center justify-between rounded-md border border-gray-300 bg-white px-3 py-2 text-sm ring-offset-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 dark:border-gray-800 dark:bg-gray-950 dark:ring-offset-gray-950 dark:placeholder:text-gray-400 dark:focus:ring-blue-500",
        className
      )}
      {...props}
    >
      ...
      <SelectPrimitive.Icon>
        <ChevronDownIcon className="h-4 w-4 opacity-50" /> {/* Use className for size */}
      </SelectPrimitive.Icon>
    </SelectPrimitive.Trigger>
  ))
  ...
  >(({ className, ...props }, ref) => (
    <SelectPrimitive.ScrollUpButton
      ...
      {...props}
    >
      <ChevronUpIcon className="h-4 w-4" /> {/* Use className for size */}
    </SelectPrimitive.ScrollUpButton>
  ))
  ...
  >(({ className, ...props }, ref) => (
    <SelectPrimitive.ScrollDownButton
      ...
      {...props}
    >
      <ChevronDownIcon className="h-4 w-4"/> {/* Use className for size */}
    </SelectPrimitive.ScrollDownButton>
  ))
    ...
    >
      <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
        <SelectPrimitive.ItemIndicator>
          <CheckIcon className="h-4 w-4" /> {/* Use className for size */}
        </SelectPrimitive.ItemIndicator>
      </span>
