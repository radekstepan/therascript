  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (axiosError.code === 'ECONNREFUSED') {
        console.error(
          `❗ Connection refused. Is the Ollama Docker container running and accessible at ${OLLAMA_URL}?`
        );
        console.error(
          `   Try running 'docker compose ps' or check Docker Desktop.`
        );
        ...
        if (
          ...
          if (
            ...
            console.error(
              `❗ Model "${payload.model}" not found. Make sure it's pulled.`
            );
            // Use the imported variable here
            console.error(
              `  Use the 'Pull Model' option or run: docker compose exec ${OLLAMA_SERVICE_NAME} ollama pull ${payload.model}`
            );
          }
        }
    ...
    throw new Error(
      `Failed to get response from Ollama. Is the server running and model '${payload.model}' available?`
    );
  }
}
...
export async function listLocalModels(): Promise<string[]> {
  try {
    ...
    if (axios.isAxiosError(error) && error.code === 'ECONNREFUSED') {
      console.error(
        `❗ Connection refused. Is the Ollama Docker container running at ${OLLAMA_URL}?`
      );
    } else {
