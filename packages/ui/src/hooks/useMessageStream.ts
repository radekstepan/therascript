  ...
  useEffect(() => {
    return () => {
      ...
      if (streamControllerRef.current) {
        console.log(
          '[useMessageStream Cleanup] Aborting stream controller on unmount.'
        );
  ...
  const processStream = useCallback(
    async (
      ...
      try {
          ...
          for (const line of lines) {
            ...
            if (line.startsWith('data:')) {
              ...
              try {
                ...
                if (
                  data.userMessageId &&
                  actualUserMessageId === -1 &&
                  tempUserMsgId !== undefined
                ) {
                ...
                // 3. Done Signal: Stream finished from the backend
                  ...
                  console.log(
                    "[useMessageStream] Stream received 'done' signal. Tokens:",
                    data
                  );
                ...
                // 4. Error Signal: Backend reported an error during stream generation
                  ...
                  console.error(
                    '[useMessageStream] SSE Error Event Received:',
                    data.error
                  );
                ...
                console.error(
                  '[useMessageStream] SSE JSON parse error:',
                  e,
                  'Raw line:',
                  line
                );
        ...
        console.error(
          '[useMessageStream] Error reading or processing stream:',
          error
        );
        if (!isCancelledRef.current) {
          // Only mark as errored if not manually cancelled
          streamErrored = true;
        ...
        if (!streamErrored && !isCancelledRef.current) {
          ...
          console.log(
            '[useMessageStream Finally] Stream completed without error.'
          );
          ...
          console.log(
            '[useMessageStream Finally] Stream processing cancelled by user.'
          );
          ...
          console.warn(
            '[useMessageStream Finally] Stream ended with an error.'
          );
    ...
    [queryClient, chatQueryKey, onStreamComplete, onStreamError]
  ); // Dependencies
