...
export const addSessionChatMessageStream = async (
    ...
    if (!response.ok || !response.body) {
        ...
        } catch { /* Ignore parsing error if body isn't JSON */ }
...
export const addStandaloneChatMessageStream = async (chatId: number, text: string): Promise<{ userMessageId: number; stream: ReadableStream<Uint8Array> }> => {
    ...
    if (!response.ok || !response.body) {
        let errTxt = `HTTP error ${response.status}`;
        try {
            const e = await response.json(); errTxt = e?.message || JSON.stringify(e);
        } catch {}
        throw new Error(`Failed to initialize stream: ${errTxt}`);
    }
...
export const updateMessageStarStatus = async (
    ...
    if (sessionId !== undefined && sessionId !== null && chatId !== undefined) {
        ...
        throw new Error("Missing required chat ID (and session ID if applicable) for star update.");
    }
