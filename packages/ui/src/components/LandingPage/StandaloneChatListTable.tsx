...
interface ChatRowCellsProps {
  chat: StandaloneChatListItem;
  onEditChatRequest: (chat: StandaloneChatListItem) => void;
  onDeleteChatRequest: (chat: StandaloneChatListItem) => void;
}

const ChatRowCells = React.memo(function ChatRowCells({
  chat,
  onEditChatRequest,
  onDeleteChatRequest,
}: ChatRowCellsProps) {
...
export function StandaloneChatListTable({
  ...
  const handleRowClick = useCallback(
    (e: React.MouseEvent<HTMLTableRowElement>, chatId: number) => {
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLTableRowElement>, chatId: number) => {
      if (
        e.key === 'Enter' &&
  ...
  const renderHeaderRow = useCallback(
    ...
    [getHeaderCellProps, renderSortIcon]
  );
  ...
  return (
    <TableVirtuoso
      ...
      data={chats}
      computeItemKey={(_index, chat) => chat.id}
      fixedHeaderContent={renderHeaderRow}
      itemContent={(_index, chat) => {
        return (
          <ChatRowCells
            chat={chat}
            onEditChatRequest={onEditChatRequest}
      ...
      components={{
        ...
        TableRow: ({ children, ...props }) => {
          const chat = chats[props['data-index']];
          if (!chat) return <tr {...props}>{children}</tr>;
          return (
            <tr
              {...props}
