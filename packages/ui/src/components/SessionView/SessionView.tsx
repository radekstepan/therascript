...
export function SessionView() {
    ...
    const { data: sessionMetadata, isLoading: isLoadingSessionMeta, error: sessionMetaError, isFetching: isFetchingSessionMeta } = useQuery<Session, Error>({
        ...
        queryFn: () => { if (!sessionIdNum) return Promise.reject(new Error("Invalid Session ID")); console.log(`[SessionView] Fetching sessionMeta for ID: ${sessionIdNum}`); return fetchSession(sessionIdNum); },
        enabled: !!sessionIdNum, staleTime: 5 * 60 * 1000,
    });
    ...
    const { data: transcriptContent, isLoading: isLoadingTranscript, error: transcriptError } = useQuery<StructuredTranscript, Error>({
        ...
        queryFn: () => { if (!sessionIdNum) return Promise.reject(new Error("Invalid Session ID")); console.log(`[SessionView] Fetching transcript for ID: ${sessionIdNum}`); return fetchTranscript(sessionIdNum); },
        enabled: !!sessionIdNum, staleTime: Infinity,
    });
    ...
    useEffect(() => {
         ...
         if (!currentSessionIdNum || isNaN(currentSessionIdNum)) { navigate('/', { replace: true }); setActiveSessionId(null); setActiveChatId(null); return; }
         setActiveSessionId(currentSessionIdNum);
         const isNewSession = previousSessionIdRef.current !== currentSessionIdNum;
         if (sessionMetadata) {
             if (isNewSession) { previousSessionIdRef.current = currentSessionIdNum; }
             const chats = sessionMetadata.chats || [];
             let targetChatId: number | null = null; let shouldNavigate = false; let navigateTo: string | null = null;
             ...
             else if (chats.length > 0) { const sortedChats = [...chats].sort((a, b) => b.timestamp - a.timestamp); targetChatId = sortedChats[0].id; if (!chatIdParam || !chatExistsInSession) { shouldNavigate = true; navigateTo = `/sessions/${currentSessionIdNum}/chats/${targetChatId}`; } }
             else if (chatIdParam) { shouldNavigate = true; navigateTo = `/sessions/${currentSessionIdNum}`; }
             if (targetChatId !== activeChatId) { console.log(`[SessionView Effect] Setting activeChatId to ${targetChatId} (was ${activeChatId})`); setActiveChatId(targetChatId); }
             if (shouldNavigate && navigateTo) { console.log(`[SessionView Effect] Navigating to ${navigateTo}`); navigate(navigateTo, { replace: true }); }
         } else { if (isNewSession) { setActiveChatId(null); previousSessionIdRef.current = currentSessionIdNum; } }
    ...
    useEffect(() => { return () => { if (isResizing.current) { document.removeEventListener('mousemove', handleMouseMove); document.removeEventListener('mouseup', handleMouseUp); document.body.style.cursor = ''; document.body.style.userSelect = ''; isResizing.current = false; } }; }, [handleMouseMove, handleMouseUp]);
     ...
     const handleStartFirstChat = useCallback(async () => { if (startChatMutation.isPending) return; startChatMutation.mutate(); }, [startChatMutation]);
     const handleOpenEditMetadataModal = () => setIsEditingMetadata(true);
     const handleOpenLlmModal = () => setIsLlmModalOpen(true);
     const handleNavigateBack = () => navigate('/');
    const handleMetadataSaveSuccess = (updatedMetadata: Partial<SessionMetadata>) => { console.log("[SessionView] Metadata save successful (via callback):", updatedMetadata); };


    // --- Render Logic ---
    if (isLoadingSessionMeta && !sessionMetadata) { return (<Flex justify="center" align="center" style={{ height: '100vh', backgroundColor: 'var(--color-panel-solid)' }}><Spinner size="3" /><Text ml="2" color="gray">Loading session data...</Text></Flex>); }
    if (sessionMetaError || !sessionMetadata) { return (<Flex direction="column" justify="center" align="center" style={{ height: '100vh', backgroundColor: 'var(--color-panel-solid)' }}><Text color="red" mb="4">{sessionMetaError?.message || "Session data could not be loaded."}</Text><Button onClick={handleNavigateBack} variant="soft" color="gray"><ArrowLeftIcon /> Go back to Sessions</Button></Flex>); }
    if (ollamaError) { console.error("Ollama status check failed:", ollamaError); /* TODO: Maybe show a non-blocking warning */ }
    ...
    return (
        <Flex flexGrow="1" style={{ height: '100vh', overflow: 'hidden' }}>
            {/* Sidebar */}
            <Box ref={sidebarRef} className="relative flex-shrink-0 hidden lg:flex flex-col" style={{ width: `${clampedSidebarWidth}px`, backgroundColor: 'var(--color-panel-solid)', borderRight: '1px solid var(--gray-a6)' }} >
                <SessionSidebar session={sessionMetadata ?? null} isLoading={isLoadingSessionMeta || isFetchingSessionMeta} error={sessionMetaError ?? null} />
            </Box>
            ...
            {/* Edit Metadata Modal */}
            <EditDetailsModal isOpen={isEditingMetadata} onOpenChange={setIsEditingMetadata} session={sessionMetadata} onSaveSuccess={handleMetadataSaveSuccess} />
