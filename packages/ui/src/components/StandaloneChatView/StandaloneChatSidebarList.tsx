...
export function StandaloneChatSidebarList({ chats, onRenameChatRequest: onEditChatRequest, onDeleteChatRequest, activeChatId }: StandaloneChatSidebarListProps) {
    const navigate = useNavigate();

    // Click/Keydown handlers remain the same
    const handleChatClick = (e: React.MouseEvent<HTMLDivElement>, chatId: number) => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>, chatId: number) => {
                ...
                {chats.map((chat) => {
                    ...
                    return (
                        <Flex
                            ...
                            onClick={(e: React.MouseEvent<HTMLDivElement>) => handleChatClick(e, chat.id)}
                            onKeyDown={(e: React.KeyboardEvent<HTMLDivElement>) => handleKeyDown(e, chat.id)}
                            tabIndex={0}
                            role="link"
                        ...
                        >
                            ...
                            <Flex align="center" gap="2" className="flex-grow pr-1" style={{ minWidth: 0 }}>
                                <ChatBubbleIcon className={cn("text-[--gray-a10] flex-shrink-0", isActive && "text-[--accent-11]")} />
