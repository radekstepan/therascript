...
async function setup() {
  console.log(
    `[Autostart Setup] Attempting to configure autostart for ${SERVICE_NAME}...`
  );
  console.warn(
    `[Autostart Setup] This operation requires root privileges (sudo).`
  );
  ...
  console.log(
    `[Autostart Setup] Absolute path to wrapper script to be run by service: ${wrapperScriptPath}`
  );
  ...
  try {
    ...
    if (enabled) {
      console.log(
        `[Autostart Setup] Service ${SERVICE_NAME} is already enabled. No action needed.`
      );
      console.log(
        `   To update/reinstall, run 'yarn autostart:remove' first, then 'yarn autostart:setup'.`
      );
    ...
    console.log(
      `[Autostart Setup] Service not enabled. Proceeding with setup...`
    );
    await setAutostart(
    ...
    console.log(
      `[Autostart Setup] Successfully enabled service ${SERVICE_NAME}.`
    );
    console.log(
      `[Autostart Setup] The application should now start automatically on the next system boot.`
    );
    console.log(
      `[Autostart Setup] To check status: sudo systemctl status ${SERVICE_NAME}`
    );
    console.log(
      `[Autostart Setup] To see logs: sudo journalctl -u ${SERVICE_NAME}`
    );
    console.log(
      `[Autostart Setup] To disable: sudo systemctl disable --now ${SERVICE_NAME}`
    );
  } catch (error) {
