...
async function checkStatus() {
  console.log(
    `[Autostart Status Check] Checking status for service: ${SERVICE_NAME}...`
  );
  // Remind user that root privileges are usually needed for system-wide checks.
  console.warn(
    `[Autostart Status Check] This check typically requires root privileges (sudo).`
  );
  ...
  try {
    ...
    if (isEnabled) {
      ...
      console.log(
        `   The application is configured to start automatically on boot.`
      );
    } else {
      console.log(
        `❌ Autostart Status for ${SERVICE_NAME}: DISABLED or NOT FOUND`
      );
      console.log(
        `   The application is NOT configured to start automatically on boot.`
      );
    ...
    console.error(
      `[Autostart Status Check] Error checking autostart status for ${SERVICE_NAME}:`,
      error
    );
