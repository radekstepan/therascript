...
async function main() {
    ...
    try {
        ...
        console.log("   (Starting keep-alive interval - press Ctrl+C to stop dev environment)");
...
async function shutdown(signal: string) {
    ...
    if (isShuttingDown) {
        console.log(`[Whisper Manager] Shutdown already in progress, ignoring signal: ${signal}`);
        return;
    }
    isShuttingDown = true;
    ...
    if (isRunning) {
        try {
            ...
            console.log("[Whisper Manager] Whisper service container stop command issued.");
        } catch (error) {
...
process.on('exit', (code) => {
     if (!isShuttingDown) {
         ...
         console.error(`   If the Whisper container was running, consider stopping it manually ('docker compose -f ./docker-compose.yml down' from root).`);
     } else {
