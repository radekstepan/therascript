...
async function runDockerComposeCommand(command: string): Promise<string> {
  ...
  try {
    ...
    // Use standard Error
    throw new Error(
      `[Whisper Docker] Failed to run 'docker compose ${command}'. Is Docker running? Error: ${error.message}`
    ); // Throws, satisfies return path
  }
}
...
const findWhisperContainer = async (): Promise<Dockerode.Container | null> => {
  ...
  try {
    ...
    const found = containers.find((c) =>
      c.Names.some((name) => name === `/${WHISPER_CONTAINER_NAME}`)
    );
    return found ? docker.getContainer(found.Id) : null;
  } catch (error) {
    console.error(
      `[Whisper Docker Manager] Error finding container '${WHISPER_CONTAINER_NAME}':`,
      error
    );
    return null;
  }
...
async function isWhisperContainerRunning(): Promise<boolean> {
  ...
  try {
    ...
    console.log(
      `[Whisper Docker Manager] Container '${WHISPER_CONTAINER_NAME}' state: ${data.State.Status}`
    );
    return data.State.Running === true;
  } catch (error: any) {
    if (error.statusCode === 404) {
      // Check specific Dockerode error
      console.warn(
        `[Whisper Docker Manager] Container '${WHISPER_CONTAINER_NAME}' not found during inspect.`
      );
    } else {
      console.error(
        `[Whisper Docker Manager] Error inspecting container '${WHISPER_CONTAINER_NAME}':`,
        error
      );
    }
    return false;
...
async function checkServiceHealth(): Promise<boolean> {
  try {
    const response = await axios.get(WHISPER_HEALTH_URL, {
    ...
    if (!isHealthy) {
      console.warn(
        `[Whisper Health] Service responded but status was not healthy: ${response.status} - ${JSON.stringify(response.data)}`
      );
    }
    ...
    if (axios.isAxiosError(error)) {
      if (error.code === 'ECONNREFUSED') {
        console.info(
          `[Whisper Health] Health check refused (service likely still starting or stopped)...`
        );
      } else if (
        ...
        console.warn(
          `[Whisper Health] Health check timed out after ${HEALTH_CHECK_TIMEOUT}ms.`
        );
      } else {
        console.warn(
          `[Whisper Health] Health check failed (Axios Error): ${error.message}`
        );
      }
    } else {
      console.warn(
        `[Whisper Health] Health check failed with non-axios error: ${error}`
      );
    }
...
async function startWhisperService(): Promise<void> {
  console.log(
    '🐳 [Whisper Docker] Starting Whisper service via Docker Compose...'
  );
  try {
    await runDockerComposeCommand(
      `up -d --remove-orphans ${WHISPER_SERVICE_NAME}`
    );
    console.log("✅ [Whisper Docker] 'docker compose up' command issued.");
  } catch (error) {
    console.error(
      '❌ [Whisper Docker] Failed to start Whisper service (docker compose up failed).'
    );
...
export async function ensureWhisperRunning(): Promise<void> {
  console.log(
    '🐳 [Whisper Docker] Ensuring Whisper service is running and healthy...'
  );
  let attemptStart = false;

  if (await isWhisperContainerRunning()) {
    console.log(
      '✅ [Whisper Docker] Container process found. Checking health...'
    );
    if (await checkServiceHealth()) {
      console.log(
        '✅ [Whisper Health] Service is already running and healthy.'
      );
      return;
    }
    console.warn(
      '⚠️ [Whisper Health] Container process is running but service is not healthy yet. Will proceed with start/check logic.'
    );
  ...
  if (attemptStart) {
    console.log(
      '🅾️ [Whisper Docker] Attempting to start/restart Whisper service...'
    );
    await startWhisperService(); // Uses compose
  } else {
    console.log(
      'ℹ️ [Whisper Docker] Skipping start command as container process was found (but might be unhealthy).'
    );
  }

  // Health check polling loop (unchanged)
  console.log(
    `⏳ [Whisper Health] Waiting for service to become healthy (retrying up to ${HEALTH_CHECK_RETRIES} times with ${HEALTH_CHECK_DELAY / 1000}s delay)...`
  );
  let isHealthy = false;
  for (let i = 0; i < HEALTH_CHECK_RETRIES; i++) {
    await new Promise((resolve) => setTimeout(resolve, HEALTH_CHECK_DELAY));
    console.log(
      `[Whisper Health] Performing health check (Attempt ${i + 1}/${HEALTH_CHECK_RETRIES})...`
    );
    ...
    if (!(await isWhisperContainerRunning())) {
      console.error(
        '❌ [Whisper Docker] Container stopped running during health check polling! Check internal container logs.'
      );
      isHealthy = false;
  ...
  if (!isHealthy) {
    const errorMsg =
      'Failed to confirm Whisper service health after start attempt.';
    console.error(
      '❌ [Whisper Health] Whisper service did not become healthy after multiple retries.'
    );
    console.error(
      '   >>> Please check the container logs: docker compose logs whisper <<<'
    );
    // Use standard Error
...
export async function stopWhisperService(): Promise<void> {
  ...
  console.log(
    `[Whisper Docker Manager] Attempting to stop container '${WHISPER_CONTAINER_NAME}'...`
  );
  try {
    const container = await findWhisperContainer();
    if (container) {
      try {
        console.log(
          `[Whisper Docker Manager] Stopping container '${WHISPER_CONTAINER_NAME}' (ID: ${container.id})...`
        );
        await container.stop(); // Configurable timeout? Add { t: 10 } for 10s timeout
        console.log(
          `✅ [Whisper Docker Manager] Container '${WHISPER_CONTAINER_NAME}' stopped.`
        );
        ...
        if (stopError.statusCode === 304) {
          console.log(
            `[Whisper Docker Manager] Container '${WHISPER_CONTAINER_NAME}' already stopped.`
          );
        } else if (stopError.statusCode === 404) {
          console.log(
            `[Whisper Docker Manager] Container '${WHISPER_CONTAINER_NAME}' not found (already removed?).`
          );
        } else {
          console.error(
            `[Whisper Docker Manager] Error stopping container '${WHISPER_CONTAINER_NAME}':`,
            stopError
          );
        }
      }
      ...
      console.log(
        `[Whisper Docker Manager] Container '${WHISPER_CONTAINER_NAME}' not found, cannot stop.`
      );
    }
  } catch (error) {
    console.error(
      `[Whisper Docker Manager] General error stopping Whisper service:`,
      error
    );
  }
}
