...
console.warn("--- This approach is likely not used by the main application. ---");
console.warn("--- The primary interaction is via the FastAPI service (server.py) ---");
...
class WhisperAPI {
  ...
  public async startJob({ audioFile, outputFile, modelName }: JobConfig): Promise<JobHandle> {
    console.warn("[DEPRECATED] WhisperAPI.startJob called. Use API service instead.");
    ...
    if (existsSync(this.cidFile)) {
      ...
      if (isRunning) {
        console.log(`Stopping potentially stale container: ${existingContainerId}`);
        await this.stopExistingContainer(existingContainerId);
      }
      console.log("Removing old container_id.txt...");
      try { unlinkSync(this.cidFile); } catch (e) { console.warn("Failed to remove cidfile:", e); }
    }
    // --- End Cleanup ---
    ...
    const args = [
      "run",
      "--gpus", "all",         // Attempt to use all available GPUs
      "--init",                // Use tini as init process for better signal handling
      "--rm",                  // Automatically remove container on exit
      "--cidfile", this.cidFile, // Write container ID to this file
      // Volume mounts:
      "-v", `${audioPath}:/input.mp3:ro`, // Mount audio file read-only
      "-v", `${outputDirOnHost}:${outputDirMountPath}`, // Mount host output dir to container output dir
      "-v", `${cachePath}:/root/.cache`,    // Mount host cache dir to container cache dir
    ...
    const parseVerboseLine = (line: string): JobStatus | null => {
      ...
      const match = line.match(/\[(\d{1,2}:\d{2}\.\d{3})\s*-->\s*(\d{1,2}:\d{2}\.\d{3})\]/);
    ...
    const promise: Promise<string> = new Promise((resolve, reject) => {
      process.on("close", (code: number | null) => {
        ...
        if (existsSync(this.cidFile)) {
          try { unlinkSync(this.cidFile); } catch(e) { console.warn("Failed to remove cidfile on close:", e);}
        }
        // Check exit code
        if (code === 0) {
      ...
      process.on("error", (err) => { // Handle errors spawning the process itself
          console.error("Error spawning Docker process:", err);
          if (existsSync(this.cidFile)) { // Cleanup cidfile on spawn error too
              try { unlinkSync(this.cidFile); } catch(e) { console.warn("Failed remove cidfile on spawn error:", e);}
          }
    ...
    const cancel = async (): Promise<void> => {
      ...
      if (process && !process.killed) { // Check if the Node.js process object exists and isn't marked as killed
        if (containerId) { // If we captured the container ID
          ...
          if (running) {
            ...
            console.log("Container associated with job is not running (already stopped or removed).");
          }
        } else {
          ...
          console.log("Container ID not available. Attempting to kill spawn process...");
        ...
        statusCallbacks.forEach(cb => cb({ status: "canceled", message: "Cancellation requested." }));
      } else {
...
const demo = async () => {
  ...
  try {
    ...
    onStatus((status: JobStatus) => {
      ...
      switch (true) {
        ...
        case status.status === "error":
          console.error(`[Demo Status] Error: ${status.message} (Code: ${status.code || 'N/A'})`);
          break;
        default:
    ...
    promise
        .then(() => console.log("[Demo Promise] Transcription successful (check output file)."))
        .catch((err: Error) => console.error("[Demo Promise] Transcription failed:", err.message));
