...
const SessionAndChatParamsSchema = t.Intersect([SessionIdParamSchema, ChatIdParamSchema]); // Corrected Intersect usage
const SessionChatAndMessageParamsSchema = t.Intersect([SessionIdParamSchema, ChatIdParamSchema, MessageIdParamSchema]);
...
export const chatRoutes = new Elysia({ prefix: '/api/sessions/:sessionId/chats' })
    ...
    .guard({ params: 'sessionIdParam' }, (app) => app
        ...
        .group('', { detail: { tags: ['Chat'] } }, (app) => app
            ...
            .guard({ params: 'sessionAndChatParams' }, (app) => app // *** FIX: Use combined schema name ***
                ...
                // --- Routes requiring :messageId ---
                 .guard({ params: 'sessionChatAndMessageParams' }, (app) => app // Nested guard for message ID
                    .derive(({ params, chatData }) => {
                        ...
                        if (!message || message.chatId !== chatData.id) {
                            ...
                            else console.error(`[Derive Session Message] Error: Message ${messageId} found, but belongs to chat ${message.chatId}, not ${chatData.id}.`);
                    ...
                    .patch('/:chatId/messages/:messageId', updateSessionChatMessageStarStatus, {
                        body: 'messageStarUpdateBody',
                        response: { 200: 'chatMessageResponse' }, // Return the updated message
                        detail: { summary: 'Update star status/name for a message' }
                    })
                ) // End message guard
            ) // End chat guard
        ) // End /chats group
