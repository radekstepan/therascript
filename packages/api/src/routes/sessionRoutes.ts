...
const SessionChatMetadataSchema = t.Omit(
  t.Object({
    // Use t.Omit if ChatMetadata includes tags
    id: t.Number(),
    sessionId: t.Number(),
    timestamp: t.Number(),
    name: t.Optional(t.Union([t.String(), t.Null()])),
    tags: t.Optional(t.Any()), // Include tags temporarily for Omit
  }),
  ['tags']
); // Omit tags for session-specific chat metadata
...
const sessionRoutesInstance = new Elysia({ prefix: '/api' })
    ...
    (app) =>
      app
        ...
        .post(
          ...
          async ({ body, set }) => {
            ...
            try {
              ...
              newSess = sessionRepository.create(
                { ...metadata, date },
                audioFile.name,
                /* REMOVED transcriptPath NULL */ null,
                ts
              );
              if (!newSess)
                throw new InternalServerError('Failed create initial session');
              ...
              if (!finalSess)
                throw new InternalServerError('Failed update status/jobId');
              set.status = 202;
              return {
                sessionId: finalSess.id,
                jobId,
                message: 'Upload ok, transcription started.',
              };
            } catch (err) {
              const origErr =
                err instanceof Error ? err : new Error(String(err));
              console.error('[API Err] Upload:', origErr.message);
              if (!(err instanceof ApiError)) console.error(origErr.stack);
              if (newSess?.id) {
                try {
                  ...
                  if (current) {
                    if (savedAudioId) {
                      try {
                        ...
                        console.error(
                          `Cleanup audio err ${savedAudioId}:`,
                          delErr
                        );
                      }
                    }
                  ...
                  console.error(
                    `Cleanup err session ${newSess.id}:`,
                    cleanupErr
                  );
                }
              }
          ...
          {
            beforeHandle: ({ body }) => {
              const f = body.audioFile;
              const maxS = parseSize(config.upload.maxFileSize);
              if (!config.upload.allowedMimeTypes.includes(f.type))
                throw new BadRequestError(`Invalid type: ${f.type}`);
              if (f.size > maxS)
                throw new BadRequestError(
                  `File size > ${config.upload.maxFileSize}`
                );
              if (f.size === 0) throw new BadRequestError('File empty.');
            },
            body: 'uploadBody',
            response: {
              202: t.Object({
        ...
        .guard({ params: 'sessionIdParam' }, (app) =>
          app
            .derive(({ params }) => {
              ...
              if (isNaN(sid)) throw new BadRequestError('Invalid ID');
              const s = sessionRepository.findById(sid);
              if (!s) throw new NotFoundError(`Session ID ${sid}`);
              return { sessionData: s };
            })
            .get(
              '/:sessionId',
              ({ sessionData, set }) => getSessionDetails({ sessionData, set }),
              {
            ...
            .put(
              '/:sessionId/metadata',
              ({ sessionData, body, set }) =>
                updateSessionMetadata({ sessionData, body, set }),
              {
            ...
            .get(
              ...
              ({ sessionData, set }) => getTranscript({ sessionData, set }),
              {
            ...
            .patch(
              '/:sessionId/transcript',
              ({ sessionData, body, set }) =>
                updateTranscriptParagraph({ sessionData, body, set }),
              {
            ...
            .post(
              ...
              async ({ params, set, sessionData }) => {
                ...
                console.log(
                  `[API Finalize] Request received for session ${sessionId}`
                );
                if (sessionData.status !== 'transcribing') {
                  throw new ConflictError(
                    `Session ${sessionId} status is '${sessionData.status}', not 'transcribing'.`
                  );
                }
                ...
                try {
                  ...
                  const structuredTranscript =
                    await getStructuredTranscriptionResult(jobId);
                  ...
                  transcriptRepository.insertParagraphs(
                    sessionId,
                    structuredTranscript
                  );
                  console.log(
                  ...
                  const finalizedSession = sessionRepository.updateMetadata(
                    sessionId,
                    {
                      status: 'completed',
                  ...
                  if (!chatsMetadata || chatsMetadata.length === 0) {
                    ...
                    messageRepository.addMessage(
                      newFullChat.id,
                      'ai',
                      `Session "${finalizedSession.sessionName}" uploaded on ${finalizedSession.date.split('T')[0]} has been transcribed...`
                    );
                    // --- END FIX ---
                    console.log(
                      `[API Finalize] Initial chat created (ID: ${newFullChat.id}) for session ${sessionId}.`
                    );
                    // --- FIX: Exclude tags when fetching/assigning chat metadata ---
                  ...
                  console.log(
                    `[API Finalize] Session ${sessionId} finalized successfully.`
                  );
                  set.status = 200;
                  ...
                  console.error(
                    `[API Error] Finalize Session ${sessionId}:`,
                    error
                  );
                  try {
                    ...
                    console.log(
                      `[API Finalize] Marked session ${sessionId} as 'failed'.`
                    );
                  } catch (updateError) {
                    console.error(
            ...
            .delete(
              ...
              async ({ params, set, sessionData }) => {
                ...
                try {
                  ...
                  if (audioId) {
                    ...
                    console.warn(`[API Del] No audio id for session ${sid}.`);
                  }
                  // Deleting the session via repository will CASCADE DELETE paragraphs
                  const deleted = sessionRepository.deleteById(sid);
                  if (!deleted)
                    console.warn(
                      `[API Del] Session DB ${sid} not found during delete attempt.`
                    );
                  else
                    console.log(
                      `[API Del] Deleted session DB ${sid} (and associated paragraphs via cascade).`
                    );
                  set.status = 200;
              ...
              },
              {
                ...
                detail: { summary: 'Delete session, files, chats, paragraphs' },
              }
            )
            ...
            .delete(
              '/:sessionId/audio',
              ({ sessionData, set }) =>
                deleteSessionAudioHandler({ sessionData, set }),
              {
                response: { 200: 'deleteResponse', 404: t.Any(), 500: t.Any() },
                detail: { summary: 'Delete original audio file' },
              }
            )
  ...
  .get(
    ...
    ({ set }) => {
      set.status = 501;
      return { message: 'API schema definition unavailable. Use /api/docs.' };
    },
    { detail: { tags: ['Meta'] } }
  );
