...
const SessionMetadataUpdateBodySchema = t.Partial(t.Object({
    ...
    // Still accept YYYY-MM-DD from UI for now, API will convert
...
const SessionMetadataResponseSchema = t.Object({
    ...
    date: t.String(), // Response is ISO string
...
const SessionListResponseItemSchema = SessionMetadataResponseSchema; // Date is ISO string
...
const UploadBodySchema = t.Object({
    ...
    // Still accepts YYYY-MM-DD from UI
...
export const sessionRoutes = new Elysia({ prefix: '/api' })
    ...
    .group('/transcription', { detail: { tags: ['Transcription'] } }, (app) => app
        .get('/status/:jobId', async ({ params }) => { /* ... (no change) ... */
            const { jobId } = params;
            try {
    ...
    .group('/sessions', { detail: { tags: ['Session'] } }, (app) => app
        .get('/', listSessions, {
            response: { 200: t.Array(SessionListResponseItemSchema) }, // Returns ISO date strings now
            detail: { summary: 'List all sessions (metadata only)' }
        })
        // POST /api/sessions/upload (Modified)
        .post('/upload', async ({ body, set }) => {
            // Note: body.date is "YYYY-MM-DD" here due to UploadBodySchema
            ...
            try {
                ...
                // *** 3. Create Session in DB using CURRENT time as timestamp ***
                const creationTimestamp = new Date().toISOString();
                newSession = await sessionRepository.create(
                    { ...metadata, date }, // Pass original metadata including YYYY-MM-DD date from form
                    audioFile.name,
                    null,
                    creationTimestamp // Pass precise creation timestamp
                );
                if (!newSession) throw new InternalServerError('Failed to create initial session record.');

                 // 4. Update session status and job ID
                 const updatedSession = await sessionRepository.updateMetadata(newSession.id, {
                    status: 'transcribing',
                    whisperJobId: jobId,
                    // Optionally update the 'date' field again here if needed,
                    // but create already used the precise timestamp.
                 });
                 if (!updatedSession) throw new InternalServerError(`Failed to update status for session ${newSession.id}.`);

                console.log(`[API Upload] DB session record created (ID: ${updatedSession.id}, Date: ${updatedSession.date}) with status 'transcribing'.`);
                ...
                return { sessionId: updatedSession.id, jobId: jobId, message: "Upload successful, transcription started." };
                 ...
                 if (newSession?.id) {
                     ...
                     try { await sessionRepository.deleteById(newSession.id); console.log(`[API Upload Cleanup] DB record deleted.`); }
                     catch (cleanupError) { console.error(`[API Upload Cleanup] Error deleting DB record:`, cleanupError); }
                 }
                  if (error instanceof ApiError) throw error;
                  const errorMessage = error instanceof Error ? error.message : 'Unknown error during upload';
                  throw new InternalServerError(`Session upload failed: ${errorMessage}`, error instanceof Error ? error : undefined);
             } finally { /* ... (temp file logic unchanged) ... */
                 if (tempAudioPath) {
                     console.log(`[API Upload] Temporary audio file ${tempAudioPath} kept for Whisper processing.`);
                 }
             }
        }, {
            beforeHandle: ({ body, set }) => { /* ... (validation unchanged) ... */
        ...
        .guard({ params: 'sessionIdParam' }, (app) => app
             ...
             .get('/:sessionId', getSessionDetails, {
                 response: { 200: 'sessionWithChatsMetadataResponse' }, // Includes ISO date
                 detail: { summary: 'Get session metadata & chat list' }
             })
             // Update metadata handler needs to convert date
             .put('/:sessionId/metadata', updateSessionMetadata, {
                 body: 'metadataUpdateBody',
                 response: { 200: 'sessionMetadataResponse' }, // Includes ISO date
             ...
             .post('/:sessionId/finalize', async ({ params, set, sessionData }) => { /* ... (no change to finalization logic) ... */
                  const sessionId = sessionData.id;
                  console.log(`[API Finalize] Request received for session ${sessionId}`);
                  if (sessionData.status !== 'transcribing') { /* ... */ throw new ConflictError(`...`); }
                  if (!sessionData.whisperJobId) { /* ... */ throw new InternalServerError(`...`); }
                  ...
                  try {
                      ...
                      const finalizedSession = await sessionRepository.updateMetadata(sessionId, { status: 'completed', transcriptPath: finalTranscriptPath, });
                      if (!finalizedSession) throw new InternalServerError(`...`);
                      ...
                      if (!initialChat) { /* ... create initial chat ... */
                           initialChat = await chatRepository.createChat(sessionId);
                           await chatRepository.addMessage(initialChat.id, 'ai', `Session "${finalizedSession.sessionName}" uploaded on ${finalizedSession.date.split('T')[0]} has been transcribed...`);
                      ...
                      if (!finalSessionState) throw new InternalServerError(`...`);
                      ...
                      try { await sessionRepository.updateMetadata(sessionId, { status: 'failed' }); console.log(`[API Finalize] Marked session ${sessionId} as 'failed'.`); }
                      catch (updateError) { console.error(`[API Finalize] CRITICAL: Failed to mark session ${sessionId} as failed:`, updateError); }
                      if (error instanceof ApiError) throw error;
             ...
             .delete('/:sessionId', async ({ params, set, sessionData }) => { /* ... (no change) ... */
                 const sessionId = sessionData.id;
                 console.log(`[API Delete] Request received for session ${sessionId}`);
                 try {
                     if (sessionData.transcriptPath) { await deleteTranscriptFile(sessionId); }
                     else { console.log(`[API Delete] Session ${sessionId} has no transcript file.`); }
                     const deleted = sessionRepository.deleteById(sessionId);
                     if (!deleted) { throw new NotFoundError(`...`); }
                     console.log(`[API Delete] Successfully deleted session ${sessionId}.`);
                     console.warn(`[API Delete] Cannot reliably determine original uploaded audio path...`);
                     set.status = 200;
