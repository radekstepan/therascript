...
const PullStatusResponseSchema = t.Object({
    ...
    status: t.Enum(
        {
            queued: 'queued', parsing: 'parsing', downloading: 'downloading', verifying: 'verifying',
            completed: 'completed', failed: 'failed', canceling: 'canceling', canceled: 'canceled'
...
export const ollamaRoutes = new Elysia({ prefix: '/api/ollama' })
    ...
    .group('', { detail: { tags: ['Ollama'] } }, (app) => app
        ...
        .get('/pull-status/:jobId', ({ params, set }) => {
            ...
            try {
                const status: OllamaPullJobStatus | null = getPullModelJobStatus(jobId); // Service returns internal type
                if (!status) { throw new NotFoundError(`Pull job with ID ${jobId} not found.`); }
                set.status = 200;
                // Return object conforming to PullStatusResponseSchema
                return {
                    jobId: status.jobId, modelName: status.modelName, status: status.status,
                    message: status.message, progress: status.progress, completedBytes: status.completedBytes,
                    totalBytes: status.totalBytes, startTime: status.startTime, endTime: status.endTime,
                    error: status.error,
                };
             }
        ...
        .post('/cancel-pull/:jobId', ({ params, set }) => {
            ...
            try {
                ...
                if (!cancelled) {
                    const jobStatus: OllamaPullJobStatus | null = getPullModelJobStatus(jobId);
                    if (!jobStatus) { throw new NotFoundError(`Pull job with ID ${jobId} not found.`); }
                    else { throw new ConflictError(`Cannot cancel job ${jobId}, status is ${jobStatus.status}.`); }
                }
                set.status = 200; return { message: `Cancellation request sent for job ${jobId}.` };
             }
        ...
        .get('/status', async ({ query, set }) => {
            ...
            try {
                 ...
                 if (loadedModelResult && 'status' in loadedModelResult && loadedModelResult.status === 'unavailable') {
                     return { status: 'unavailable', activeModel: currentActiveModel, modelChecked: modelNameToCheck, loaded: false, details: undefined, configuredContextSize: currentConfiguredContext, };
                 } else {
                     const loadedModelInfo = loadedModelResult as OllamaModelInfo | null;
                      // --- FIX: Convert Date objects back to strings for API response ---
                     ...
                     return { status: 'available', activeModel: currentActiveModel, modelChecked: modelNameToCheck, loaded: !!loadedModelInfo, details: detailsResponse, configuredContextSize: currentConfiguredContext, };
                     // --- End FIX ---
                  }
             }
