...
import { Elysia, t, ValidationError, type Context as ElysiaContext, type Static } from 'elysia'; // Ensure Context is imported
...
import { ApiError, InternalServerError, ConflictError, BadRequestError, NotFoundError } from './errors.js'; // Added NotFoundError and ConflictError
...
import {
    ...
    deleteOllamaModel as deleteOllamaModelService, // <-- Import delete service function
...
// --- NEW Schemas for Delete ---
const DeleteModelBodySchema = t.Object({
    modelName: t.String({ minLength: 1, error: "Model name is required." })
});
const DeleteModelResponseSchema = t.Object({
    message: t.String()
});
// --- END NEW ---
...
const app = new Elysia()
    ...
    .group('/api/ollama', { detail: { tags: ['Ollama'] } }, (app) => app
        ...
        .post('/set-model', async ({ body, set }) => {
            ...
            body: 'setModelBody',
            response: { 200: t.Object({ message: t.String() }), 400: t.Any(), 500: t.Any() },
        ...
        .post('/unload', async ({ set }) => {
            ...
            try { await ollama.chat({ model: modelToUnload, messages: [{ role: 'user', content: 'unload request' }], keep_alive: 0, stream: false, }); console.log(`[API Unload] Sent unload request (keep_alive: 0) for active model ${modelToUnload}`); set.status = 200; return { message: `Unload request sent for model ${modelToUnload}. It will be unloaded shortly if idle.` }; }
             ...
             response: { 200: t.Object({ message: t.String() }), 500: t.Any() },
        ...
        .post('/pull-model', ({ body, set }) => {
            ...
            detail: { summary: 'Initiate downloading a new Ollama model (poll status)' }
        })
        ...
        .get('/pull-status/:jobId', ({ params, set }) => {
            ...
            detail: { summary: 'Get the status and progress of an ongoing Ollama model pull job' }
        })
        ...
        .post('/cancel-pull/:jobId', ({ params, set }) => {
            ...
            params: 'jobIdParam',
            response: { 200: 'cancelPullResponse', 404: t.Any(), 409: t.Any(), 500: t.Any(), },
            detail: { summary: 'Attempt to cancel an ongoing Ollama model pull job' }
        })

        // --- NEW: Delete Model Endpoint ---
        .post('/delete-model', async ({ body, set }) => {
            ...
            try {
                ...
                return { message: resultMessage };
                 ...
                 console.error(`[API DeleteModel] Error deleting model ${modelName}:`, error);
                 if (error instanceof ApiError) throw error; // Re-throw known API errors (e.g., NotFoundError, ConflictError)
                 throw new InternalServerError(`Failed to delete model ${modelName}.`, error);
            }
        }, {
            body: 'deleteModelBody', // Use the new schema
            response: {
                 200: 'deleteModelResponse', // Use the new schema
                 400: t.Any(), // e.g., Invalid model name format
                 404: t.Any(), // e.g., Model not found locally
                 409: t.Any(), // e.g., Conflict - trying to delete loaded model? (Handled in service)
                 500: t.Any(), // e.g., Docker command failed
            },
            detail: {
                summary: 'Delete a locally downloaded Ollama model',
            }
        })
        // --- END NEW ---
