...
try {
  ...
  console.error(
    'FATAL: Failed to prepare transcript repository statements:',
    e
  );
...
export const transcriptRepository = {
  ...
  findParagraphsBySessionId: (sessionId: number): StructuredTranscript => {
    if (!selectParagraphsStmt)
      throw new Error('Select paragraphs statement not prepared.');
    try {
   ...
   */
  insertParagraphs: transaction(
    (sessionId: number, paragraphs: StructuredTranscript): void => {
      if (!insertParagraphStmt)
        throw new Error('Insert paragraph statement not prepared.');
      ...
      try {
        ...
        for (const para of paragraphs) {
          // para is of type TranscriptParagraphData
          // Execute the prepared insert statement with mapped values
          insertParagraphStmt.run(
            sessionId,
            para.id,
            para.timestamp,
            para.text
  ...
  updateParagraphText: (
    ...
    if (!updateParagraphStmt)
      throw new Error('Update paragraph statement not prepared.');
    try {
  ...
  deleteParagraphsBySessionId: (sessionId: number): boolean => {
    if (!deleteParagraphsStmt)
      throw new Error('Delete paragraphs statement not prepared.');
    try {
