...
export const messageRepository = {
    ...
    addMessage: ( chatId: number, sender: 'user' | 'ai', text: string, promptTokens?: number | null, completionTokens?: number | null ): BackendChatMessage => {
        const timestamp = Date.now(); // Timestamp the message creation
         try {
            ...
            console.log(`[MessageRepo AddMessage] Inserted message ${newId} for chat ${chatId}. FTS trigger should have fired.`);
            return newMsg;
         } catch (error) {
    ...
    updateMessageStarStatus: ( messageId: number, starred: boolean, starredName?: string | null ): BackendChatMessage | null => {
        try {
            ...
            if (info.changes === 0) {
                console.warn(`[MessageRepo] No message found with ID ${messageId} to update star status.`);
                return null;
            }
            ...
            if (!updatedMessage) {
                ...
                console.error(`[MessageRepo] CRITICAL: Failed to retrieve message ${messageId} immediately after star update.`);
                return null;
            }
            console.log(`[MessageRepo UpdateStar] Updated star status for message ${messageId} to starred=${starredIntValue}, name=${nameToSave}`);
