// This file is a rename of the original transcriptionService.real.ts
// It contains the logic for the asynchronous, job-based Whisper service.
...
const WHISPER_API_URL = config.transcription.whisper.apiUrl;
const WHISPER_MODEL_TO_USE = config.transcription.whisper.model;

console.log('[Real Service] Using Real Whisper Transcription Service');

export async function checkTranscriptionSvcHealth(): Promise<boolean> {
  try {
    console.log(
      `[Real WhisperService] Pinging Whisper health at ${WHISPER_API_URL}/health`
    );
    await axios.get(`${WHISPER_API_URL}/health`, { timeout: 3000 });
    console.log(`[Real WhisperService] Whisper health check successful.`);
    return true;
  } catch (error) {
    if (axios.isAxiosError(error) && error.code === 'ECONNREFUSED') {
      console.warn(
        `[Real WhisperService] Whisper health check failed: Connection refused at ${WHISPER_API_URL}.`
      );
    } else {
      console.warn(
        `[Real WhisperService] Whisper health check failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
    return false;
  }
...
export const startTranscriptionJob = async (
  ...
  console.log(
    '[Real WhisperService] Unloading active Ollama model to free up GPU memory for transcription...'
  );
  try {
    await unloadActiveModel();
    console.log(
      '[Real WhisperService] Unload request sent to Ollama service successfully.'
    );
  } catch (error) {
    console.warn(
      `[Real WhisperService] Could not unload Ollama model. Continuing with transcription. Error: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  console.log('[Real WhisperService] Checking Whisper service availability...');
  const isHealthy = await checkTranscriptionSvcHealth();
  if (!isHealthy) {
    console.error(
      `[Real WhisperService] Whisper service at ${WHISPER_API_URL} is not available.`
    );
    throw new ApiError(
      503,
  ...
  console.log(
    '[Real WhisperService] Whisper service is available. Proceeding with job submission.'
  );

  console.log(
    `[Real WhisperService] Requesting transcription job for: ${fileName} via ${WHISPER_API_URL} using model '${WHISPER_MODEL_TO_USE}'`
  );
  ...
  try {
    ...
    console.log(
      `[Real WhisperService] Sending audio file and model name to ${WHISPER_API_URL}/transcribe ...`
    );
    ...
    if (submitResponse.status !== 202 || !submitResponse.data.job_id) {
      console.error(
        `[Real WhisperService] Unexpected response from Whisper submission: ${submitResponse.status}`,
        submitResponse.data
      );
    ...
    console.log(
      `[Real WhisperService] Job submitted successfully. Job ID: ${jobId}`
    );
    return jobId;
  } catch (error: any) {
    console.error(
      '[Real WhisperService] Error submitting job to Whisper:',
      error
    );
    if (axios.isAxiosError(error)) {
      ...
      if (axiosError.response) {
        console.error(
          '[Real WhisperService] Whisper Submit Error Response:',
...
export const getTranscriptionStatus = async (
  ...
  console.log(`[Real WhisperService] Checking status for job ${jobId}...`);
  try {
    ...
    if (
      ...
      console.error(
        `[Real WhisperService] Invalid status response for job ${jobId}:`,
        statusResponse.data
      );
      throw new InternalServerError(
    ...
    console.error(
      `[Real WhisperService] Error polling status for job ${jobId}:`,
      error
    );
    if (axios.isAxiosError(error)) {
      const axiosError = error as AxiosError;
...
export const getStructuredTranscriptionResult = async (
  ...
  console.log(
    `[Real WhisperService] Fetching and structuring result for completed job ${jobId}...`
  );
  ...
  if (jobStatus.status !== 'completed') {
    console.error(
      `[Real WhisperService] Cannot get result: Job ${jobId} status is '${jobStatus.status}' (error: ${jobStatus.error || 'none'}).`
    );
    throw new InternalServerError(
  ...
  if (!jobStatus.result?.segments) {
    console.warn(
      `[Real WhisperService] Job ${jobId} completed but result or segments are missing. Status object:`,
      jobStatus
    );
    throw new InternalServerError(
  ...
  console.log(
    `[Real WhisperService] Got ${jobStatus.result.segments.length} segments for job ${jobId}. Grouping into paragraphs...`
  );
  const structuredTranscript = groupSegmentsIntoParagraphs(
    jobStatus.result.segments
  );
  console.log(
    `[Real WhisperService] Grouped into ${structuredTranscript.length} paragraphs for job ${jobId}.`
  );
