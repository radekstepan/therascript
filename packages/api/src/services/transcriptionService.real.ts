...
import { InternalServerError, ApiError, BadRequestError, NotFoundError } from '../errors.js'; // Custom error classes
import type {
...
async function checkWhisperApiHealth(): Promise<boolean> {
    try {
        ...
        if (axios.isAxiosError(error) && error.code === 'ECONNREFUSED') {
             console.warn(`[Real TranscriptionService] Whisper health check failed: Connection refused at ${WHISPER_API_URL}.`);
        } else {
...
function groupSegmentsIntoParagraphs(segments: WhisperSegment[]): StructuredTranscript {
    ...
    segments.forEach((segment, index) => {
        ...
        const shouldSplit = index === segments.length - 1
                          || timeGapMs > 1000
...
export const startTranscriptionJob = async (filePath: string): Promise<string> => {
    ...
    if (!isHealthy) {
        ...
        throw new ApiError(503, `Transcription service is currently unavailable at ${WHISPER_API_URL}. Please ensure it is running and accessible.`);
    }
    ...
    try {
        ...
        if (submitResponse.status !== 202 || !submitResponse.data.job_id) { // Expecting 202 Accepted
            console.error(`[Real TranscriptionService] Unexpected response from Whisper submission: ${submitResponse.status}`, submitResponse.data);
            throw new InternalServerError('Failed to submit transcription job to Whisper service. Invalid response received.');
        }
...
export const getTranscriptionStatus = async (jobId: string): Promise<WhisperJobStatus> => {
    ...
    try {
        ...
        const statusResponse = await axios.get<WhisperJobStatus>(`${WHISPER_API_URL}/status/${jobId}`, { timeout: 10000 }); // 10s timeout

        // Basic validation of the response structure
        if (!statusResponse.data || !statusResponse.data.job_id || !statusResponse.data.status) {
            console.error(`[Real TranscriptionService] Invalid status response for job ${jobId}:`, statusResponse.data);
        ...
        if (axios.isAxiosError(error)) {
            ...
            if (axiosError.response?.status === 404) {
                ...
                throw new NotFoundError(`Job ID ${jobId} not found on Whisper service.`);
            }
...
export const getStructuredTranscriptionResult = async (jobId: string): Promise<StructuredTranscript> => {
    ...
    if (jobStatus.status !== 'completed') {
        console.error(`[Real TranscriptionService] Cannot get result: Job ${jobId} status is '${jobStatus.status}' (error: ${jobStatus.error || 'none'}).`);
    ...
    if (!jobStatus.result?.segments) {
        console.warn(`[Real TranscriptionService] Job ${jobId} completed but result or segments are missing. Status object:`, jobStatus);
        throw new InternalServerError('Transcription completed but no segments were returned by the Whisper service.');
    }
