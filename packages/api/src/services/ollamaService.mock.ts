...
export const checkModelStatus = async (
  ...
  if (modelToCheck === mockLoadedModel) {
    ...
    console.log(
      `[Mock Ollama] Model '${modelToCheck}' is currently loaded (mock).`
    );
...
export const unloadActiveModel = async (): Promise<string> => {
  ...
  if (mockLoadedModel === modelToUnload) {
    ...
    console.log(
      `[Mock Ollama] Successfully unloaded model '${modelToUnload}'.`
    );
    ...
    console.log(
      `[Mock Ollama] Model '${modelToUnload}' was not the loaded model ('${mockLoadedModel}'). No action taken.`
    );
...
export const ensureOllamaReady = async (timeoutMs = 30000): Promise<void> => {
  console.log(
    '[Mock Ollama] Ensuring Ollama service is ready (mock)... always ready.'
  );
...
export const reloadActiveModelContext = async (): Promise<void> => {
  ...
  console.log(
    `[Mock Ollama] Attempting to reload context for active model: ${modelName} (mock).`
  );
...
export const streamChatResponse = async (
  ...
  async function* generateMockStream(): AsyncIterable<ChatResponse> {
    ...
    console.log(
      `[Mock Ollama Stream] Finished streaming for: "${lastUserMessage.substring(0, 30)}..."`
    );
  }
...
export const startPullModelJob = (modelName: string): string => {
  ...
  console.log(
    `[Mock Ollama] Queuing pull job ${jobId} for model: ${modelName}`
  );
  const jobStartTime = Date.now();
  const jobData: OllamaPullJobStatus = {
    jobId,
    modelName,
    status: 'queued',
    message: 'Pull queued',
...
export const getPullModelJobStatus = (
  ...
  console.log(
    `[Mock Ollama] Getting status for pull job ${jobId}. Found: ${!!status}`
  );
...
export const cancelPullModelJob = (jobId: string): boolean => {
  ...
  if (!job) {
    console.log(`[Mock Ollama] Cancel request for non-existent job ${jobId}`);
    return false;
  }
  if (
    ...
    console.log(
      `[Mock Ollama] Job ${jobId} is already terminal (${job.status}), cannot cancel.`
    );
    return false;
  }
