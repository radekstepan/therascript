...
try {
  ...
  console.error(
    '[DockerService] Failed to initialize Dockerode. Docker daemon might not be running or accessible:',
    error
  );
 ...
 */
export const getProjectContainerStatus = async (): Promise<
  DockerContainerStatus[]
> => {
  // Check if Dockerode client was initialized successfully
  if (!docker) {
    console.error(
      '[DockerService] Docker client is not initialized. Cannot fetch container status.'
    );
    throw new InternalServerError(
      'Docker client unavailable. Cannot connect to Docker daemon.'
    );
  }

  console.log(
    '[DockerService] Fetching status for project containers:',
  ...
  try {
    ...
    const projectContainers = allContainers.filter((c) =>
      PROJECT_CONTAINER_NAMES.some((expectedName) =>
        c.Names.includes(`/${expectedName}`)
      )
    );

    console.log(
      `[DockerService] Found ${projectContainers.length} matching containers in Docker.`
    );
    ...
    for (const expectedName of PROJECT_CONTAINER_NAMES) {
      if (!containerStatuses.some((cs) => cs.name === expectedName)) {
        console.log(
          `[DockerService] Expected container '${expectedName}' not found.`
        );
        containerStatuses.push({
          id: 'N/A',
          name: expectedName,
    ...
    console.log(
      '[DockerService] Successfully fetched and processed container statuses.'
    );
    ...
    console.error(
      '[DockerService] Error fetching container status from Docker daemon:',
      error
    );
    ...
    if (
      ...
      throw new InternalServerError(
        'Connection refused: Cannot connect to the Docker daemon.'
      );
    }
    // Wrap other errors
    throw new InternalServerError(
