...
try {
    ...
    shutdownScriptPath = path.resolve(systemPackageDir, 'dist/shutdownTrigger.js');
    ...
    if (!shutdownScriptPath.includes('system') || !shutdownScriptPath.includes('dist')) {
        throw new Error("Calculated path seems incorrect. Check relative path assumptions.");
    }
} catch (error) {
...
export const executeShutdown = async (): Promise<{ message: string }> => {
    ...
    try {
        ...
        if (error.stdout) console.error(`[SystemService Shutdown] STDOUT: ${error.stdout}`);
        ...
        if (error.message?.includes('command not found') || error.code === 127) {
            throw new InternalServerError(`Shutdown command failed: Node executable or script not found at specified path. Command: ${command}`, error);
        }
        // Check stderr for the classic sudo password prompt.
        if (error.stderr?.includes('sudo: a password is required') || error.stderr?.includes('sudo: no tty present and no askpass program specified')) {
        ...
        if (error.message?.includes('permission denied') || error.code === 1) { // Exit code 1 can sometimes indicate permission issues with sudo
             throw new ApiError(503, 'Shutdown failed: Permission denied executing script or sudo command.', error.message);
        }
